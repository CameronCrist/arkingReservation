# 部署指南 - Private Parking Reservation System

本文档详细说明了如何部署和验证机密停车位预订系统智能合约。

## 📋 目录

- [前置要求](#前置要求)
- [环境配置](#环境配置)
- [本地开发](#本地开发)
- [测试网部署](#测试网部署)
- [主网部署](#主网部署)
- [合约验证](#合约验证)
- [部署后操作](#部署后操作)
- [故障排除](#故障排除)

---

## 🔧 前置要求

### 必需工具

- **Node.js**: v18.0.0 或更高版本
- **npm**: v8.0.0 或更高版本
- **Git**: 最新版本
- **MetaMask**: 浏览器钱包扩展

### 检查环境

```bash
node --version    # 应显示 v18.x 或更高
npm --version     # 应显示 v8.x 或更高
npx hardhat --version  # 应显示 Hardhat 版本
```

---

## ⚙️ 环境配置

### 1. 克隆项目

```bash
git clone <your-repository-url>
cd 
```

### 2. 安装依赖

```bash
npm install
```

### 3. 配置环境变量

复制环境变量模板：

```bash
cp .env.example .env
```

编辑 `.env` 文件并填入以下信息：

```env
# 部署账户私钥（从 MetaMask 导出）
PRIVATE_KEY=your_private_key_without_0x_prefix

# Sepolia 测试网 RPC URL
SEPOLIA_RPC_URL=https://sepolia.infura.io/v3/YOUR_INFURA_API_KEY

# Etherscan API Key（用于合约验证）
ETHERSCAN_API_KEY=YOUR_ETHERSCAN_API_KEY

# 启用 Gas 报告（可选）
REPORT_GAS=true
```

### 4. 获取测试 ETH

对于 Sepolia 测试网，从以下水龙头获取测试 ETH：

- **Sepolia Faucet**: https://sepoliafaucet.com/
- **Alchemy Faucet**: https://sepoliafaucet.com/
- **Infura Faucet**: https://www.infura.io/faucet/sepolia

确保您的钱包至少有 0.1 ETH 用于部署。

---

## 🏠 本地开发

### 1. 编译合约

```bash
npm run compile
```

输出应显示：

```
Compiled 1 Solidity file successfully
```

### 2. 运行测试

```bash
npm test
```

### 3. 启动本地网络

在一个终端窗口中：

```bash
npm run node
```

这将启动本地 Hardhat 网络，监听在 `http://127.0.0.1:8545`

### 4. 部署到本地网络

在另一个终端窗口中：

```bash
npm run deploy:local
```

### 5. 运行模拟脚本

```bash
npx hardhat run scripts/simulate.js --network localhost
```

---

## 🌐 测试网部署

### Sepolia 测试网部署

#### 步骤 1: 准备部署

确保您的 `.env` 文件已正确配置：

```bash
# 检查环境变量
cat .env
```

#### 步骤 2: 编译合约

```bash
npm run compile
```

#### 步骤 3: 部署合约

```bash
npm run deploy -- --network sepolia
```

或使用脚本：

```bash
npx hardhat run scripts/deploy.js --network sepolia
```

#### 步骤 4: 保存部署信息

部署成功后，脚本会自动保存部署信息到：

```
deployments/sepolia-PrivateParkingReservationV2.json
```

部署信息包含：

- 合约地址
- 部署者地址
- 部署时间
- 交易哈希
- 网络信息

#### 示例输出

```
============================================================
开始部署机密停车位预订合约...
============================================================

📋 部署信息:
────────────────────────────────────────────────────────────
部署账户: 0x1234...5678
账户余额: 0.5 ETH
网络名称: sepolia
Chain ID: 11155111
────────────────────────────────────────────────────────────

🚀 正在部署合约: PrivateParkingReservationV2...
📦 开始部署交易...
⏳ 等待合约部署确认...

✅ 合约部署成功！
────────────────────────────────────────────────────────────
合约地址: 0x78257622318fC85f2a9c909DD7aF9d0142cd90ce
────────────────────────────────────────────────────────────

🔗 区块链浏览器链接:
────────────────────────────────────────────────────────────
Etherscan: https://sepolia.etherscan.io/address/0x78257622318fC85f2a9c909DD7aF9d0142cd90ce
────────────────────────────────────────────────────────────
```

---

## 🔍 合约验证

### 自动验证（推荐）

部署后，使用验证脚本：

```bash
npx hardhat run scripts/verify.js --network sepolia
```

### 手动验证

```bash
npx hardhat verify --network sepolia <CONTRACT_ADDRESS>
```

### 验证成功标志

```
✅ 合约验证成功！
────────────────────────────────────────────────────────────
📊 查看已验证的合约代码:
https://sepolia.etherscan.io/address/0x78257622318fC85f2a9c909DD7aF9d0142cd90ce#code
```

---

## 🚀 主网部署

### ⚠️ 重要提醒

1. **审计合约**: 在主网部署前，必须进行专业的安全审计
2. **测试充分**: 确保在测试网上进行充分测试
3. **备份密钥**: 安全备份您的私钥
4. **准备资金**: 确保有足够的 ETH 支付 Gas 费用

### 主网部署步骤

1. **更新 `.env` 配置**

```env
PRIVATE_KEY=your_mainnet_private_key
MAINNET_RPC_URL=https://mainnet.infura.io/v3/YOUR_API_KEY
ETHERSCAN_API_KEY=YOUR_ETHERSCAN_API_KEY
```

2. **添加主网配置到 `hardhat.config.ts`**

```typescript
mainnet: {
  url: process.env.MAINNET_RPC_URL || "",
  accounts: process.env.PRIVATE_KEY ? [process.env.PRIVATE_KEY] : [],
  chainId: 1,
}
```

3. **部署到主网**

```bash
npx hardhat run scripts/deploy.js --network mainnet
```

4. **验证合约**

```bash
npx hardhat run scripts/verify.js --network mainnet
```

---

## 🔧 部署后操作

### 1. 更新前端配置

将部署的合约地址更新到前端配置文件：

**public/config.js**:

```javascript
const CONFIG = {
  contractAddress: "0x78257622318fC85f2a9c909DD7aF9d0142cd90ce",
  networkId: 11155111, // Sepolia
  networkName: "sepolia"
};
```

### 2. 更新 ABI

部署脚本会自动保存 ABI 到：

```
deployments/PrivateParkingReservationV2-ABI.json
```

将此文件复制到前端：

```bash
cp deployments/PrivateParkingReservationV2-ABI.json public/contracts/
```

### 3. 测试合约交互

使用交互脚本测试合约功能：

```bash
npx hardhat run scripts/interact.js --network sepolia
```

### 4. 运行完整模拟

```bash
npx hardhat run scripts/simulate.js --network sepolia
```

---

## 📊 当前部署信息

### Sepolia 测试网

| 项目 | 信息 |
|------|------|
| **合约地址** | `0x78257622318fC85f2a9c909DD7aF9d0142cd90ce` |
| **网络** | Sepolia Testnet |
| **Chain ID** | 11155111 |
| **区块浏览器** | [查看合约](https://sepolia.etherscan.io/address/0x78257622318fC85f2a9c909DD7aF9d0142cd90ce) |
| **验证状态** | ✅ 已验证 |
| **部署时间** | 2024-XX-XX |

### 在线演示

- **前端应用**: https://private-parking-reservation.vercel.app/
- **GitHub 仓库**: https://github.com/CameronCrist/PrivateParkingReservation

---

## 🛠️ 故障排除

### 常见问题

#### 1. "Insufficient funds" 错误

**问题**: 账户余额不足

**解决方案**:
```bash
# 检查账户余额
npx hardhat run scripts/check-balance.js --network sepolia

# 从水龙头获取测试 ETH
# https://sepoliafaucet.com/
```

#### 2. "Invalid API Key" 错误

**问题**: Etherscan API Key 无效

**解决方案**:
```bash
# 检查 .env 文件中的 API Key
cat .env | grep ETHERSCAN_API_KEY

# 从 Etherscan 获取新的 API Key
# https://etherscan.io/apis
```

#### 3. "Contract already verified" 消息

**问题**: 合约已经被验证过

**解决方案**: 这不是错误，合约已成功验证。

#### 4. "Nonce too high" 错误

**问题**: 交易 nonce 不匹配

**解决方案**:
```bash
# 重置 MetaMask 账户
# Settings -> Advanced -> Reset Account
```

#### 5. Gas 价格过高

**问题**: 网络拥堵导致 Gas 费用高

**解决方案**:
```bash
# 在 hardhat.config.ts 中调整 Gas 设置
gasPrice: "auto",
gas: "auto",

# 或等待网络不那么拥堵时部署
```

### 获取帮助

如果遇到其他问题：

1. 检查 [Hardhat 文档](https://hardhat.org/docs)
2. 查看 [GitHub Issues](https://github.com/CameronCrist/PrivateParkingReservation/issues)
3. 加入社区讨论

---

## 📝 部署检查清单

### 部署前

- [ ] 安装所有依赖 (`npm install`)
- [ ] 配置 `.env` 文件
- [ ] 获取足够的测试 ETH
- [ ] 编译合约 (`npm run compile`)
- [ ] 运行测试 (`npm test`)
- [ ] 在本地网络测试部署

### 部署中

- [ ] 执行部署脚本
- [ ] 保存合约地址
- [ ] 记录交易哈希
- [ ] 等待足够的确认

### 部署后

- [ ] 验证合约代码
- [ ] 更新前端配置
- [ ] 复制 ABI 文件
- [ ] 测试合约交互
- [ ] 运行模拟脚本
- [ ] 更新 README 文档
- [ ] 提交代码到 Git

---

## 🔐 安全建议

1. **私钥管理**
   - 永远不要提交 `.env` 文件到版本控制
   - 使用硬件钱包进行主网部署
   - 定期轮换 API Keys

2. **合约安全**
   - 进行专业安全审计
   - 使用 Slither 等工具进行静态分析
   - 实施多签钱包进行管理

3. **网络安全**
   - 使用可信的 RPC 提供商
   - 启用 API Key 限流
   - 监控异常交易活动

---

## 📚 相关资源

- [Hardhat 官方文档](https://hardhat.org/)
- [Ethers.js 文档](https://docs.ethers.org/)
- [Zama FHE 文档](https://docs.zama.ai/)
- [Sepolia 测试网信息](https://sepolia.dev/)
- [Etherscan API 文档](https://docs.etherscan.io/)

---

## 🎯 后续步骤

1. **监控合约**: 使用 Etherscan 监控合约活动
2. **优化 Gas**: 分析和优化合约 Gas 消耗
3. **扩展功能**: 添加新功能和改进
4. **社区反馈**: 收集用户反馈并迭代

---

**最后更新**: 2024-XX-XX
**维护者**: Zama FHE Developer Team
**许可证**: MIT License
